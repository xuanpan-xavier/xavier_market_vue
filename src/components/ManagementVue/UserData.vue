<template>
  ...
  <el-card>
    ...
    <el-row>
      <el-col :span="8">
        <el-input placeholder="请输入查询用户名" clearable v-model="queryInfo.Telephone" @clear="getUser">
          <el-button slot="append" icon="el-icon-search" @click="getUser" ></el-button>
        </el-input>
      </el-col>
    </el-row>
      <h2 class="VIP_label">会员数据简报</h2>
    <h2 class="VIP_label" v-if="!queryInfo.Telephone">请输入查询用户的电话号码</h2>
      <div
        class="echart1"
        id="mychart1"
        :style="{ float: 'left', width: '50%', height: '400px' }"
      ></div>
      <h4 class="VIP_label2" v-if="queryInfo.Telephone">近期消费记录</h4>
      <div
        class="echart2"
        id="mychart2"
        :style="{ float: 'left', width: '50%', height: '400px' }"
...
export default {
  ...
  methods: {
    ...
    async initEcharts1 () {
      const { data: res } = await this.$http.get('Order/pie', { params: this.queryInfo })
    ...
    async initEcharts2 () {
      const { data: res } = await this.$http.get('Order/bar', { params: this.queryInfo })
      this.$message.success('获取用户数据成功!')
